...
El corte operativo de Astro usa `GET /api/events/:eventId/closeout` y queda reservado para `supervisor` y `admin`. Es una lectura calculada del estado actual del evento; todavia no bloquea ventas ni cambia el estado del evento. La ruta acepta filtros opcionales `distributorId` y `userId` para generar cortes por responsable o por usuario vinculado. La pantalla permite imprimir o descargar en CSV el corte visible.
